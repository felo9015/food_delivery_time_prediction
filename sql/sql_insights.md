...
**A data-driven minimum-deliveries threshold, not a fixed number.** Before running the regression, couriers are filtered to those with "enough" data to make a trend estimate meaningful — but instead of hard-coding a number (which would only happen to be reasonable for this dataset's specific size and volume), the threshold is computed from the data itself: couriers are ranked by delivery volume, and the cutoff is set to the smallest delivery count found among the top 80% of couriers by volume (equivalently, the 20th percentile of the per-courier delivery-count distribution). This makes the threshold adjustable to whatever data is actually available, rather than depending on an invented constant. On this synthetic dataset it produces a threshold of 123 deliveries out of 15 couriers — which is stricter than it might first appear: it excludes the one courier that was deliberately built with a real increasing trend (119 deliveries, just under the cutoff), so the final significant-trend list comes back empty. That is reported as an honest result, not adjusted away — see `sql_exploration.ipynb`, Question 5, for the full reasoning. It is a useful, real trade-off to flag for a production version of this analysis: a principled, reproducible threshold can be conservative enough to filter out a borderline-but-genuine signal along with the noise it is designed to exclude, which argues for treating "just below the cutoff" couriers as candidates for closer monitoring rather than as confirmed non-trends.

## GenAI Usage Disclosure

**Tool used:** Claude Code, Anthropic's coding assistant.

**What it was used for in Part I:** drafting and iterating SQL syntax for all 5 queries; explaining the semantics of specific clauses (`JOIN` ON-conditions and their failure modes, `WHERE` vs. `HAVING`, multi-column `GROUP BY`, window functions) while building each query incrementally in the exploration notebook; and prototyping the Question 5 trend-detection methodology, including the initial window-function (`LAG()`) approach, its later replacement with a per-delivery `scipy.stats.linregress` regression, and the design of the synthetic dataset used to validate every query end to end.

**How the output was validated and modified:** every query was run against the synthetic dataset in `sql_exploration.ipynb` and checked against deliberately seeded edge cases (e.g., an area with a resolved historical problem vs. one with a recent spike for Question 1; an inactive high-volume courier and a low-volume new hire for Question 3) before being accepted — no query in `sql_queries.sql` was taken on faith. Explanatory text was reviewed cell by cell until the underlying logic was fully understood, not just copied. Two methodology choices were deliberately overridden from what was first suggested, based on independent judgment: the minimum-deliveries threshold for Question 5 was changed from an initial fixed value to the data-driven percentile-based calculation described above, once it was clear a fixed number would not generalize to different datasets; and `scipy.stats.linregress` was chosen over `numpy.polyfit` specifically to obtain a p-value, since the business question is about statistical significance, not just the sign of a slope.
